// EquiliPrism Global Audio Manager
class AudioManager {
  constructor() {
    this.audioCtx = null;
    this.analyser = null;
    ...
    this.tracks = [
      {
        ...
        description: 'Gentle, floating chords designed for calm study and focused learning.',
        type: 'synth'
      },
      {
        id: 'soft-horizon',
        title: 'Soft Horizon',
        artist: 'EquiliPrism Synth',
        description: 'A relaxing chiptune pad playing calm Major and Minor 7th chords.',
        type: 'synth'
      },
      {
        id: 'peaceful-forest',
        ...
        artist: 'Ambient Synth',
        description: 'Floating, wide chords designed for focused mathematical thinking.',
        type: 'synth'
      },
      {
        id: 'midnight-ocean',
        title: 'Midnight Ocean',
        artist: 'Dreamy Chimes',
        description: 'Deep, soothing Major 7th chimes reminiscent of gentle ocean waves.',
        type: 'synth'
      },
      {
        id: 'little-star',
        title: 'Little Star Chimes',
        artist: 'EquiliPrism Synth',
        description: 'A beautiful, child-appropriate synthesized ambient chime playing Twinkle Twinkle chords.',
        type: 'synth'
      }
    ];

    // Load custom tracks from localStorage and filter out any deprecated/unwanted references
    try {
    ...
    if (track.type === 'stream') {
      if (this.streamAudio && this.currentPlayingTrackId === track.id && !forceRestart) {
        ...
        if (playPromise !== undefined && typeof playPromise.catch === 'function') {
          playPromise.catch(e => {
            console.warn("Resume failed, restarting stream:", e);
  ...
  playStream(url) {
    ...
    this.streamAudio.loop = false; // Disable loop to support continuous play
    this.streamAudio.volume = this.volume;
    
    let hasStarted = false;
    const fallbackTimeout = setTimeout(() => {
      if (!hasStarted && this.isPlaying) {
        console.warn("Stream loading timed out, falling back to procedural synthesizer.");
        this.isLoading = false;
    ...
    this.streamAudio.addEventListener('ended', () => {
      if (this.isPlaying) {
        console.log("Stream track ended. Playing next track...");
        this.next();
      }
    });
    ...
    if (playPromise !== undefined && typeof playPromise.catch === 'function') {
      playPromise.catch(e => {
        console.warn("Stream playback failed immediately: ", e);
        this.isLoading = false;
        clearTimeout(fallbackTimeout);
        this.triggerStreamFallback();
      });
  ...
  triggerStreamFallback() {
    ...
    const fallbackTrack = this.tracks[this.currentTrackIdx];
    this.playSynth(fallbackTrack.id);
    this.triggerUpdate();
  }
  ...
  playSynth(type) {
    ...
    const playChord = () => {
      ...
      if (chordsPlayed >= 12) {
        console.log("Synth track completed. Playing next track...");
        this.next();
        return;
      }
      ...
      chord.forEach((note, idx) => {
        ...
        let attack = 2.0 + idx * 0.4;
        let decay = 7.0 + idx * 0.4;
        let waveType = idx === 0 ? 'triangle' : 'sine';
        let vol = 0.12;

        if (type === 'little-star') {
          attack = 0.05 + idx * 0.05;
          decay = 3.0 + idx * 0.3;
          waveType = 'sine';
          vol = 0.08;
        } else if (type === 'midnight-ocean') {
  ...
  addCustomTrack(title, artist, url) {
    const newTrack = {
      ...
      artist,
      url,
      description: 'Custom user-added stream track.',
      type: 'stream'
    };
    
    this.tracks.push(newTrack);
    
    // Save only custom tracks to localStorage
...
window.equiliprismAudio = audioManager; // make it accessible globally
